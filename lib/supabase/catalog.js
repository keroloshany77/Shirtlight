...
export function mapSupabaseProduct(row) {
  ...
  const availableColors = colors.filter((c) => Number(c.stock || 0) > 0);
  ...
  return {
    ...
    imageUrl: images[0] || '',
    gallery: images,
    sizeChartUrl: assetPath(row.size_chart_url || ''),
    variants,
    sizes: availableVariants.length ? availableVariants.map((variant) => variant.size) : variants.map((variant) => variant.size),
    colors: availableColors.length ? availableColors.map((c) => c.color) : colors.map((c) => c.color),
