...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const playNotificationSound = () => {
    if (!audioReadyRef.current || typeof window === 'undefined') return;
    ...
    try {
      ...
      // Browsers can block audio until user interaction; keep notifications visual.
    }
  };
  ...
  useEffect(() => {
    ...
    const loadNotifications = async () => {
      ...
      const nextIds = new Set([
        ...
        ...nextMessages.map((message: any) => `message:${message.id}`),
      ]);

      if (notificationsInitializedRef.current) {
        const hasNewNotification = [...nextIds].some((id) => !knownNotificationIdsRef.current.has(id));
  ...
  if (isLoading) return <div className="admin-shell min-h-screen bg-black text-white font-inter" />;

  if (authMessage) {
    return (
      <div className="admin-shell min-h-screen bg-black text-white font-inter flex items-center justify-center p-6 text-center">
  ...
  return (
    <div className="admin-shell min-h-screen bg-[#000] text-white font-inter flex">
      {/* SIDEBAR */}
        ...
        {isSidebarOpen && (
          ...
          >
            <div className="h-20 flex items-center px-8 border-b border-white/10">
              <span className="font-syncopate font-bold tracking-[0.3em] text-sm">SHIRT NIGHT</span>
            </div>
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="flex items-center gap-3 pl-6 border-l border-white/10">
              ...
              <div className="hidden sm:block">
                ...
                <p className="text-xs text-white/50">admin@shirtnight.shop</p>
              </div>
            </div>
          </div>
        </header>
