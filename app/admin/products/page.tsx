...
const IMPORTED_STOCK = 99;

function websiteProductToAdminProduct(product: any): Product {
  ...
  return {
    ...
    stock: sizes.length * IMPORTED_STOCK,
    sizes,
    colors,
    image: product.imageUrl || product.gallery?.[0] || '',
    description: product.description || '',
    sizeChartUrl: product.sizeChartUrl || '',
    variants: sizes.map((size: string) => ({ size, stock: IMPORTED_STOCK })),
    colorVariants: colors.map((color: string) => ({ color, stock: IMPORTED_STOCK })),
    reviews: [],
    source: 'website',
  };
}
...
export default function AdminProducts() {
  ...
  const loadProducts = async () => {
    ...
    const mappedProducts = (data || []).map((product: any) => ({
      ...
      image: assetPath(product.product_images?.[0]?.url || ''),
      description: product.description || '',
      sizeChartUrl: assetPath(product.size_chart_url || ''),
    ...
    if (mappedProducts.length === 0) {
      setProducts(websiteProducts.map(websiteProductToAdminProduct));
      setStatusMessage('Showing the current website products. Import them into Supabase to edit them from this dashboard.');
      return;
    }
  ...
  const hasWebsiteFallback = products.some((product) => product.source === 'website');

  const importWebsiteProducts = async () => {
    setStatusMessage('');
    setIsSaving(true);

    const { isAdmin } = await getCurrentAdmin();
    if (!isAdmin) {
      setStatusMessage('Sign in as an active admin before importing website products into Supabase.');
      setIsSaving(false);
      return;
    }
    ...
    try {
      ...
      setStatusMessage('Current website products imported into Supabase.');
      await loadProducts();
    } catch (error: any) {
      setStatusMessage(error?.message || 'Could not import website products.');
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleSave = async () => {
    ...
    const payload = {
      ...
      size_chart_url: formData.sizeChartUrl.trim(),
    ...
    try {
      if (editingProd) {
        if (selectedSizeChartFile) {
          ...
          const sizeChartImage = await uploadSizeChartImage(editingProd.id);
          uploadedSizeChartPath = sizeChartImage.path;
          payload.size_chart_url = sizeChartImage.url;
        }
        ...
        if (selectedSizeChartFile) {
          currentStep = 'uploading size chart to Storage';
          const sizeChartImage = await uploadSizeChartImage(productId);
          uploadedSizeChartPath = sizeChartImage.path;
          payload.size_chart_url = sizeChartImage.url;
        }
  ...
  const uploadSizeChartImage = async (productId: string) => {
    if (!selectedSizeChartFile) return { path: '', url: formData.sizeChartUrl.trim(), sortOrder: 0 };
      ...
      <div className="flex justify-between items-end">
        ...
        <div className="flex flex-wrap justify-end gap-3">
          {hasWebsiteFallback && (
            <button
              type="button"
              onClick={importWebsiteProducts}
              disabled={isSaving}
              className="border border-white/10 bg-white/5 text-white font-syncopate text-xs font-bold tracking-widest px-6 py-3 rounded-md hover:bg-white/10 transition-colors disabled:opacity-50"
            >
              {isSaving ? 'IMPORTING...' : 'IMPORT WEBSITE PRODUCTS'}
            </button>
          )}
          <button
                ...
                <tr key={prod.id} className="hover:bg-white/5 transition-colors group">
                  <td className="px-6 py-4 flex items-center gap-4">
                    <div className="w-12 h-12 bg-black border border-white/10 rounded-md overflow-hidden shrink-0 flex items-center justify-center">
                      {prod.image ? (
                        <img src={assetPath(prod.image)} alt={prod.name} className="w-full h-full object-cover" />
                      ) : (
                    ...
                    <div>
                      ...
                      {prod.source === 'website' && (
                        <p className="text-[10px] text-white/40 font-syncopate tracking-widest mt-1">WEBSITE DATA</p>
                      )}
                    </div>
                  </td>
                  ...
                  <td className="px-6 py-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      <button
                        ...
                        disabled={prod.source === 'website'}
                        className="p-2 hover:bg-white/10 rounded-md transition-colors text-white/50 hover:text-white disabled:opacity-30 disabled:cursor-not-allowed"
                        title={prod.source === 'website' ? 'Import this product to Supabase before editing' : 'Edit'}
                      >
                        <Edit2 className="w-4 h-4" />
                      </button>
                      <button
                        onClick={() => handleDelete(prod.id)}
                        disabled={isDeletingId === prod.id || prod.source === 'website'}
                        className="p-2 hover:bg-red-500/10 rounded-md transition-colors text-white/50 hover:text-red-500 disabled:opacity-40 disabled:cursor-not-allowed"
                        title={prod.source === 'website' ? 'Import this product to Supabase before deleting' : 'Delete'}
                      >
        ...
        {isModalOpen && (
          <>
            ...
            <motion.div 
              ...
              className="admin-floating-modal w-full max-w-3xl bg-[#0a0a0a] border border-white/10 rounded-xl p-6 z-[2001] max-h-[90vh] overflow-y-auto shadow-2xl"
            >
                ...
                <div className="space-y-4">
                  ...
                  <div className="border-t border-white/10 pt-4">
                    ...
                    <label htmlFor="sizeChartImage" className="border border-dashed border-white/20 rounded-xl min-h-40 flex flex-col items-center justify-center hover:bg-white/5 transition-colors cursor-pointer group overflow-hidden">
                      ...
                      />
                      {selectedSizeChartFile || formData.sizeChartUrl ? (
                        <img
                          src={selectedSizeChartFile ? URL.createObjectURL(selectedSizeChartFile) : formData.sizeChartUrl}
                        ...
                        <>
                          ...
                          <p className="text-sm font-bold">Click to upload size chart</p>
                          <p className="text-xs text-white/50 mt-1">JPEG, PNG up to 5MB</p>
                        </>
                      )}
                    </label>
                    {(selectedSizeChartFile || formData.sizeChartUrl) && (
                      <button
                        ...
                        onClick={() => {
                          setSelectedSizeChartFile(null);
                          setFormData({ ...formData, sizeChartUrl: '' });
                        }}
                        className="mt-3 text-xs font-syncopate tracking-widest text-white/50 hover:text-white transition-colors"
                      >
                        REMOVE SIZE CHART
                      </button>
                    )}
